...
- **EHW-4.1 host twin done** — `sw/ehw/memetic_kernel.h` and
  `sw/ehw/memetic_eval.c` mirror the EHW-4.0 Python oracle. The host gate compares
  Python and C per-generation curves byte-for-byte for pure GA, pure SGD,
...
- **EHW-4.3 board-verified** — the EHW-local memetic train unit passed OOC/resource
  and place gates (`18/20` DSP in the RP pblock; train unit `2` DSP after the
...
- **EHW-5.1 host twin done** — `sw/ehw/memetic_struct_kernel.h` and
  `sw/ehw/memetic_struct_eval.c` mirror the EHW-5.0/5.0b Python oracle. The host
