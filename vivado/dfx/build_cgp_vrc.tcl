...
# EHW-1.1-fabric: CGP VRC reconfigurable module (3x4 LUT4 grid behind the XBUS window)
create_reconfig_module -name rm_cgp_vrc -partition_def [get_partition_defs tpu_pd] -top tpu_rp
add_files -norecurse -of_objects [get_reconfig_modules rm_cgp_vrc] \
    [list $root/rtl/dfx/tpu_rp_rm_cgp_vrc.v $root/rtl/cgp_vrc.v]
...
create_pr_configuration -name cfg10 -partitions [list $rp_cell:rm_cgp_vrc]
...
# EHW-1.1-fabric: build static + rm_cgp_vrc (impl_10) and stop.
launch_runs impl_10 -to_step write_bitstream -jobs 8
wait_on_run impl_10
puts "=== impl_10 (cfg10 static+rm_cgp_vrc): [get_property STATUS [get_runs impl_10]] ==="
foreach b [glob -nocomplain $bdir/$proj.runs/impl_10/*.bit] { puts "  BIT: $b" }
exit
